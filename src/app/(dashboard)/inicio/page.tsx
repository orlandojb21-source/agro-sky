      ...
      <div className="grid w-full max-w-2xl grid-cols-2 gap-3 sm:grid-cols-3">
        {accesos.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className="flex flex-col items-center justify-center gap-1 rounded-xl border border-green-100 bg-white p-6 text-center font-medium text-green-900 shadow-sm transition hover:border-green-300 hover:shadow-md dark:border-green-900/40 dark:bg-green-950/10 dark:text-green-50 dark:hover:border-green-700"
          >
            {item.label}
          </Link>
        ))}
