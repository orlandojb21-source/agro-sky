...
export default async function CajaMenudaLayout({
  ...
  const [saldo, vistaPrevia] = await Promise.all([
    calcularSaldoActual(supabase),
    calcularVistaPreviaActual(supabase),
  ]);
  const necesitaReposicion = saldo <= UMBRAL_ALERTA;
  const detalle = vistaPrevia.detalle;
  const totalEnCaja = DENOMINACIONES.reduce((suma, d) => suma + (detalle[d.id] ?? 0) * d.valor, 0);
      ...
      <div className="rounded-xl border border-green-100 bg-white p-4 shadow-sm dark:border-green-900/40 dark:bg-green-950/10">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <div className="text-center sm:col-span-2">
            <p className="text-xs uppercase tracking-wide text-green-700/70 dark:text-green-300/70">
              {vistaPrevia.fechaArqueo
                ? `Efectivo en caja (arqueo del ${formatDateOnly(vistaPrevia.fechaArqueo)} + movimientos posteriores)`
                : "Efectivo en caja (aún no se ha hecho un arqueo formal)"}
            </p>
