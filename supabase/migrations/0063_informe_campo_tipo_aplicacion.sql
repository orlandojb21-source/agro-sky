-- Agro Sky — migracion 0063: nuevo campo "Tipo de Aplicacion" en Informe de Campo
-- Ejecutar una sola vez en Supabase: Dashboard > SQL Editor > New query > pegar > Run
--
-- Pedido del usuario (2026-08-05): agregar, debajo de Meteorologia, un
-- campo "Tipo de Aplicacion" con 2 opciones fijas (Liquido/Granulado).
-- Requerido, igual que el resto de los campos del encabezado.

alter table informes_campo
  add column tipo_aplicacion text check (tipo_aplicacion in ('liquido', 'granulado'));
...
--
-- IMPORTANTE (leccion de la migracion 0061): estas firmas deben
-- coincidir EXACTAMENTE con las que existen hoy en la base de datos
-- (post 0061+0062: p_dosis_por_hectarea ya es "text", no "numeric") --
-- si no coinciden, el drop no borra nada y create function deja una
-- funcion sobrecargada duplicada en vez de reemplazarla.
